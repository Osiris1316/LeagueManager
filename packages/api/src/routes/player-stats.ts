...
export function playerStatsRoutes<E extends { Bindings: { DB: D1Database } }>() {
  ...
  // Raw slots for a player + season metadata — client folds & filters (Path B).
  routes.get('/players/:playerId/slots', async (c) => {
    const db = c.env.DB;
    const playerId = parseInt(c.req.param('playerId'), 10);
    ...
    const { results: seasons } = await db
      .prepare(`
        ...
        FROM seasons ORDER BY season_number ASC
      `)
      .all();

    return c.json({ playerId, slots, seasons });
  });
